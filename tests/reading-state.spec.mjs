...
async function currentArticleRatio(page){
...
test('途中位置を保存し、ボタンとresume=1の両方から再開する',async({page})=>{
  ...
  await expect.poll(async()=>Number.parseFloat(await storageValue(page,POSITION_KEY)||'0')).toBeGreaterThan(0.4);
  const savedRatio=Number.parseFloat(await storageValue(page,POSITION_KEY));
  expect(savedRatio).toBeLessThan(0.7);
  ...
  await expect.poll(()=>currentArticleRatio(page)).toBeGreaterThan(savedRatio-0.12);
